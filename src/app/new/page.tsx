...
const New = () => {
  const [newItem, setNewItem] = useState<ItemMap | null>(null);
  ...
  const fetchNewItems = async (page: number) => {
    const res = await fetch(`/api/best-items?new=true&page=${page}&limit=${limit}`);
  ...
  return (
    <div>
      ...
      <h1 className="title-p px-4">신상품</h1>
      <section className="my-10">
        <ul className="w-full grid grid-cols-[repeat(4,1fr)] gap-x-5 gap-y-12 mb-10 px-4">
          {newItem?.data.map((item: Items) => (
            <li key={item.id}>
              <Link className="inline-block w-full rounded-2xl" href={{ pathname: `/new/${item.id}` }}>
